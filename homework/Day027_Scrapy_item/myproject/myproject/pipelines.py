...
class JSONPipeline(object):
    def open_spider(self, spider):
        ...
        self.start_crawl_datetime = datetime.now()
        # 在開始爬蟲的時候建立暫時的 JSON 檔案
        # 避免有多筆爬蟲結果的時候，途中發生錯誤導致程式停止會遺失所有檔案
        # dir_path為myproject/crawled_data的絕對路徑
        ...
        # scrapy.log已被棄用，參考https://docs.scrapy.org/en/latest/topics/logging.html#logging-from-spiders
        # spider.log('Create temp file for store JSON - {}'.format(self.runtime_file_path))
        spider.logger.info('Create temp file for store JSON - {}'.format(self.runtime_file_path))
    ...
    def close_spider(self, spider):
        ...
        self.end_crawl_datetime = datetime.now()
        ...
        self.store_file_path = self.dir_path / '{:%Y%m%dT%H-%M-%S}-{:%Y%m%d%H-%M-%S}.json'.format(
            self.start_crawl_datetime,
        ...
        spider.logger.info('Save result at {}'.format(self.store_file_path))
